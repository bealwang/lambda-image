...
from MEHI.utils.common import checkParams, handleFormat, raiseErrorIfPathExists
...
class ThunderContext():
    ...
    def makeExample(self, dataset=None, **opts):
        ...
        See MEHI.utils.datasets for detailed options.
        ...
        """
        from MEHI.utils.datasets import DATASET_MAKERS
    ...
    def loadExample(self, dataset=None):
        ...
        if 'ec2' in self._sc.master:
            tmpdir = os.path.join('/root/MEHI/MEHI/utils', 'data', DATASETS[dataset])
        else:
            tmpdir = tempfile.mkdtemp()
            atexit.register(shutil.rmtree, tmpdir)

            def copyLocal(target):
                files = resource_listdir('MEHI.utils.data', target)
                for f in files:
                    path = resource_filename('MEHI.utils.data', os.path.join(target, f))
    ...
    def loadJSON(self, path):
        ...
        import json
        from MEHI.rdds.fileio.readers import getFileReaderForPath, FileNotFoundError
        from MEHI.utils.serializable import _decode_dict
    ...
    def loadSources(self, path):
        ...
        SourceExtraction
        """
        from MEHI import SourceExtraction
    ...
    def export(self, data, filename, outputFormat=None, overwrite=False, varname=None):
        ...
        from MEHI.rdds.fileio.writers import getFileWriterForPath
    ...
    def setAWSCredentials(self, awsAccessKeyId, awsSecretAccessKey):
        ...
        launching a cluster using the MEHI-ec2 script, credentials will be
