...
def pil_to_array(pilImage):
    ...
    if pilImage.mode in ('RGBA', 'RGBX', 'RGB'):
        raise ValueError("MEHI only supports luminance / greyscale images in pil_to_array; got image mode: '%s'" %
                         pilImage.mode)
    if pilImage.mode == 'L':
        ...
        raise ValueError("MEHI only supports luminance / greyscale images in pil_to_array; got unknown image " +
...
def handleFormat(filename, format):
    ...
    """
    import os
    from MEHI.utils.context import DEFAULT_EXTENSIONS
...
def raiseErrorIfPathExists(path, awsCredentialsOverride=None):
    ...
    from MEHI.rdds.fileio.readers import getFileReaderForPath
