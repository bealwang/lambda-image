...
conf = SparkConf().setAppName('test').setMaster('local[4]').set('spark.executor.memory','2g').set('spark.driver.maxResultSize','6g').set('spark.driver.memory','8g').set('spark.local.dir','/dev/shm').set('spark.storage.memoryFraction','0.2').set('spark.default.parallelism','10')
...
def func1():
    ...
    rddA = tsc.loadImages('/mnt/lambda_image/1-L/*.tif', inputFormat='tif-stack')
